...
def run(config):
    ...
    #     save_top_k=1
    # )
    # best_callback._state_key = "ModelCheckpoint:best_ckpt"  # ⭐️ 添加这行
    callbacks = [
        lr_monitor,
        checkpoint_callback,
        # best_callback,
    ...
    if config.ckpt and config.ckpt.endswith(".ckpt"):
        ...
        # ➕ 这里插入 best.ckpt 的拷贝逻辑
        ckpt_files = [f for f in os.listdir(save_dir) if f.endswith(".ckpt") and "val" in f]
        if ckpt_files:
            ckpt_files.sort(reverse=True)
            best_ckpt = ckpt_files[0]
            best_ckpt_path = os.path.join(save_dir, best_ckpt)
            best_ckpt_copy_path = os.path.join(save_dir, "best.ckpt")
            shutil.copy(best_ckpt_path, best_ckpt_copy_path)
            print(f"✔️ Copied {best_ckpt} to best.ckpt")
        else:
            print("❌ No valid checkpoint file found to copy as best.ckpt")
