...
class FeaturizeLightningModel(LightningModule):
    ...
    def __init__(self, cfg):
        ...
        # print("=======================================features_dir get=======================================")
        # print(self.cfg.dataset.output_dir)
        if not os.path.isdir(self.features_dir):
            os.makedirs(self.features_dir)

        # Load metadata from config path
        if hasattr(cfg.dataset, 'metadata_path'):
            self.df_metadata = pd.read_csv(cfg.dataset.metadata_path, sep="\t")
        else:
    ...
    def shared_step(self, batch, split, extract_features=True):
...
#             for ids, f in zip(instance_id, features.cpu().detach().numpy()):
#                 try:
#                     # 获取 image_id（一个 CT 的唯一标识）
#                     clean_id = ids.split('@')[0]
#                     cleaned_image_id = self.df_metadata[self.df_metadata['impression_id'] == int(clean_id)]['image_id'].iloc[0]
#                     cleaned_image_id = cleaned_image_id.replace('.nii.gz', '')
#                     feature_path = os.path.join(self.features_dir, f"{cleaned_image_id}.npy")

#                     if os.path.exists(feature_path):
#                         # 如果已经存在，加载旧的 feature，append 当前这个
#                         old_feature = np.load(feature_path)
#                         new_feature = np.vstack([old_feature, f])
#                     else:
#                         new_feature = f.reshape(1, -1)

#                     np.save(feature_path, new_feature)
#                 except Exception as e:
#                     print("[ERROR]", ids, str(e))
#                     continue
...
#                 df_data[f'feature_{i}'] = all_features[:, i]
#             df = pd.DataFrame(df_data)
#             df["impression_id"] = df["instance_id"].apply(lambda x: x.split("@")[0]).astype(int)

#             output_path = Path(self.features_dir)
#             output_path.mkdir(parents=True, exist_ok=True)
#             df.to_csv(output_path / f"{prefix}_features.csv", index=False)
#             print(f"✔️ CSV 保存完成，共 {len(df)} 条记录，特征维度 {all_features.shape[1]}")

#             # 保存为 .npy（按 CT 汇总）
#             features_by_impression = defaultdict(list)
#             for feature, instance_id in zip(all_features, all_instance_ids):
#                 impression_id = int(instance_id.split("@")[0])
#                 features_by_impression[impression_id].append(feature)

#             for impression_id, feature_list in features_by_impression.items():
#                 try:
#                     feature_array = np.stack(feature_list)
#                     image_id = self.df_metadata[self.df_metadata['impression_id'] == impression_id]['image_id'].iloc[0]
#                     image_id = image_id.replace('.nii.gz', '')
#                     np.save(output_path / f"{image_id}.npy", feature_array)
#                 except Exception as e:
#                     print(f"[ERROR] Failed to process {impression_id}: {e}")
#                     continue
#             print(f"✔️ NPY 保存完成，共 {len(features_by_impression)} 个 CT 文件")

#         else:
#             print("⚠️ Warning: No features collected during epoch")
